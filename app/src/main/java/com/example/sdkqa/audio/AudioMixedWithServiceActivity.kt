...
class AudioMixedWithServiceActivity : AppCompatActivity() {
    ...
    private val modes = arrayOf("Local", "Audio Simple", "Episode", "Live")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startService(createConfigForMode("Local"))
    }
    ...
    private fun createConfigForMode(mode: String): MediastreamPlayerConfig {
        ...
        when (mode) {
            "Local" -> {
                config.src = "android.resource://${packageName}/raw/sample_audio"
            ...
            "Audio Simple" -> {
                ...
                Log.d(TAG, "Config: Audio Simple (AOD) mode - id: $AOD_ID")
            }
            "Episode" -> {
    ...
    private fun startService(config: MediastreamPlayerConfig) {
        ...
        MediastreamPlayerServiceWithSync.initializeService(
            this,
            this@AudioMixedWithServiceActivity,
            config,
            container,
            playerView,
            miniPlayerConfig,
    ...
    private fun createPlayerCallback(): MediastreamPlayerCallback {
        return object : MediastreamPlayerCallback {
            ...
            override fun onError(error: String?) {
                ...
                Toast.makeText(this@AudioMixedWithServiceActivity, error, Toast.LENGTH_LONG).show()
            }
